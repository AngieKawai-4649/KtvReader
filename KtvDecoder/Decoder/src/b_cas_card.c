...
static int init_b_cas_card(void* bcas)
{
	...
	ret = KtvCardEstablishContext(SCARD_SCOPE_USER, NULL, NULL, &(prv->mng));
	...
	ret = KtvCardListReaders(prv->mng, NULL, (LPTSTR)&rp, &len);
...
static int get_id_b_cas_card(void *bcas, B_CAS_ID *dst)
{
	...
	ret = KtvCardTransmit(prv->card, SCARD_PCI_T1, prv->sbuf, slen, NULL, prv->rbuf, &rlen);
...
static int proc_emg_b_cas_card(void* bcas, uint8_t* src, int len)
{
	...
	retry_count = 0;
	ret = KtvCardTransmit(prv->card, SCARD_PCI_T1, prv->sbuf, slen, &sir, prv->rbuf, &rlen);
	while (((ret != SCARD_S_SUCCESS) || (rlen < 6)) && (retry_count < 2)) {
		retry_count += 1;
		if (!connect_card(prv, prv->reader)) {
			continue;
...
{
	if(prv->card != 0){
		KtvCardDisconnect(prv->card, SCARD_LEAVE_CARD);
		prv->card = 0;
	}

	if(prv->mng != 0){
		KtvCardReleaseContext(prv->mng);
		prv->mng = 0;
	}
...
static int connect_card(B_CAS_CARD_PRIVATE_DATA *prv, LPCTSTR reader_name)
{
	...
	if(prv->card != 0){
		KtvCardDisconnect(prv->card, SCARD_RESET_CARD);
		prv->card = 0;
	}

	ret = KtvCardConnect(prv->mng, reader_name, SCARD_SHARE_SHARED, SCARD_PROTOCOL_T1, &(prv->card), &protocol);
	if(ret != SCARD_S_SUCCESS){
		return 0;
	}
	...
	rlen = B_CAS_BUFFER_MAX;
	ret = KtvCardTransmit(prv->card, SCARD_PCI_T1, prv->sbuf, m, NULL, prv->rbuf, &rlen);
	if(ret != SCARD_S_SUCCESS){
		return 0;
	}
