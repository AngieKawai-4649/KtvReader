...
#include <fstream>
using namespace std;
ofstream debugOut("libaribb25_deb.txt");
*/

EXTERN_C HINSTANCE h_module;
EXTERN_C HANDLE usbHandle;
EXTERN_C UINT cardResponseTime;
...
const void CB25Decoder::Release()
{
	// インスタンス開放
...
const void CB25Decoder::usbHandleExport(HANDLE handle, UINT response) {
	usbHandle = handle;
	cardResponseTime = response;
}
